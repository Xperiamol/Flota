...
const postprocessMarkdown = (md) => {
  if (!md) return md
  // 支持多层嵌套引用 > > \[!type\]
  let out = md.replace(/^((?:>\s*)+)\\\[!(\w+)\\\]/gm, '$1[!$2]')
  // 还原双链：tiptap-markdown 会把 `[[xxx]]` 转义为 `\[\[xxx\]\]`，
  // 这会破坏 markdown-it wikiLink 与 useLinkGraph 的扫描，统一还原。
  out = out.replace(/\\\[\\\[([^\]\n]+?)\\\]\\\]/g, '[[$1]]')
    ...
    return [
      new Plugin({
        ...
        appendTransaction(_transactions, _oldState, newState) {
          // 光标所在的空段落：用户正在此处编辑（如刚按 Backspace 删空），
          // 不补 sentinel，否则会与删除操作互相抵消导致无法向上合并。
          const headPos = newState.selection.$head.pos
          const emptyTopLevelParagraphs = []
          newState.doc.descendants((node, pos, parent) => {
            if (parent !== newState.doc) return
            if (node.type.name === 'paragraph' && node.content.size === 0) {
              if (headPos > pos && headPos < pos + node.nodeSize) return
...
// lowlight 已注册的语言（common 集合），用于代码块语言选择下拉
const AVAILABLE_CODE_LANGUAGES = lowlight.listLanguages().sort()

// 常见语言别名 → 规范显示名（其余直接首字母大写）
const CODE_LANGUAGE_LABELS = {
  ...
  c: 'C', cpp: 'C++', csharp: 'C#', php: 'PHP', sql: 'SQL', bash: 'Bash', shell: 'Shell',
  json: 'JSON', yaml: 'YAML', xml: 'XML', html: 'HTML', css: 'CSS', scss: 'SCSS',
  markdown: 'Markdown', md: 'Markdown', dockerfile: 'Dockerfile', diff: 'Diff',
}

const formatLanguageLabel = (lang) => {
  if (!lang) return '纯文本'
  return CODE_LANGUAGE_LABELS[lang] || (lang.charAt(0).toUpperCase() + lang.slice(1))
}

// 代码块 NodeView：在右上角提供语言选择器，写回 node.attrs.language
const CodeBlockNodeView = ({ node, updateAttributes, editor }) => {
  ...
  const filteredLanguages = useMemo(() => {
    ...
    if (!kw) return AVAILABLE_CODE_LANGUAGES
    return AVAILABLE_CODE_LANGUAGES.filter((lang) =>
      lang.toLowerCase().includes(kw) || formatLanguageLabel(lang).toLowerCase().includes(kw)
    )
  }, [filter])
  ...
  return (
    ...
    >
      <Box
        ...
        onClick={() => editor.isEditable && setMenuOpen((v) => !v)}
        sx={{
          position: 'absolute',
          top: 6,
          right: 8,
      ...
      >
        {formatLanguageLabel(currentLang)}
        {editor.isEditable && <ExpandMoreIcon sx={{ fontSize: 14 }} />}
      </Box>
      ...
      {menuOpen && (
        <CodeLanguageMenu
          ...
          onClose={() => { setMenuOpen(false); setFilter('') }}
        />
      )}
...
// 语言选择浮层：用 Portal 渲染到 body，定位到锚点下方
const CodeLanguageMenu = ({ anchorEl, filter, onFilterChange, languages, currentLang, onSelect, onClose }) => {
  ...
  useLayoutEffect(() => {
    ...
    // 下方放不下且上方更宽裕时，朝上展开
    const flipUp = spaceBelow < maxMenuHeight + gap && spaceAbove > spaceBelow
    const available = flipUp ? spaceAbove - gap : spaceBelow - gap
    const height = Math.max(120, Math.min(maxMenuHeight, available - 8))
    setPos({
  ...
  return (
    <Portal>
      ...
      >
        <Box sx={{ p: 0.8, borderBottom: '1px solid', borderColor: 'divider' }}>
          <TextField
            autoFocus
            fullWidth
            size="small"
            ...
            variant="outlined"
            slotProps={{ input: { sx: { fontSize: '0.78rem', borderRadius: '8px' } } }}
          />
        </Box>
        <Box sx={{ overflowY: 'auto', py: 0.5 }}>
          ...
          {languages.length === 0 && (
            <Box sx={{ px: 1.5, py: 1, fontSize: '0.75rem', color: 'text.disabled' }}>
              无匹配语言
            </Box>
          )}
        </Box>
...
const LanguageMenuRow = ({ label, hint, active, onClick }) => (
  <Box
    onClick={onClick}
    sx={{
      display: 'flex',
  ...
  >
    <span>{label}</span>
    {hint && hint !== label.toLowerCase() && (
      <MuiTypography component="span" sx={{ fontSize: '0.68rem', color: 'text.disabled', ml: 1 }}>
        {hint}
      </MuiTypography>
    )}
  </Box>
...
// 扩展 CodeBlockLowlight，注入语言选择 NodeView（序列化方式不变）
...
const useEditorState = (editor) => {
  ...
  useEffect(() => {
    ...
    const dom = (() => {
      try { return editor.view?.dom } catch { return null }
    })()
...
const getSelectionRect = (editor) => {
  ...
  let view
  try { view = editor.view } catch { return null }
  if (!view) return null
...
const ContextMenuButton = ({ children, active = false, danger = false, disabled = false, shortcut, onClick }) => (
  <ButtonBase
    ...
    onClick={onClick}
    focusRipple
    TouchRippleProps={{ style: { color: danger ? 'rgba(239,68,68,0.45)' : active ? 'rgba(59,130,246,0.45)' : 'rgba(120,120,128,0.35)' } }}
    sx={{
      width: '100%',
      borderRadius: '8px',
      px: 1.25,
...
const moveTopLevelBlocks = (editor, blocks, movingIds, targetIndex) => {
  ...
  // 计算第一个被移动块在新文档中的起始位置，drop 后把光标放回该位置
  // 文档根开始 pos = 0，每个块占用 node.nodeSize，第一个 moving 块开头 +1 进入其内容
  const offsetBeforeMoving = nextNodes
    .slice(0, clampedTarget)
    .reduce((sum, node) => sum + node.nodeSize, 0)
  const targetSelectionPos = offsetBeforeMoving + 1

  let tr = editor.state.tr.replaceWith(0, editor.state.doc.content.size, Fragment.fromArray(nextNodes))
  try {
    const safePos = Math.min(targetSelectionPos, tr.doc.content.size)
    tr = tr.setSelection(TextSelection.near(tr.doc.resolve(safePos)))
  } catch (_) {
    // 极端文档结构异常时忽略，保持默认 selection
  }
  editor.view.dispatch(tr.scrollIntoView())
  editor.view.focus()
  return true
...
const WYSIWYGEditor = forwardRef(({ noteId, content, onChange, onEditorReady, onBlockSelectModeChange, onWikiLinkClick, placeholder = '开始输入...' }, ref) => {
  ...
  // [[ 自动补全：suggestion state（由 ProseMirror 插件回填）
  const [wikiSuggestState, setWikiSuggestState] = useState({ open: false })
  const wikiSuggestStateRef = useRef(wikiSuggestState)
  wikiSuggestStateRef.current = wikiSuggestState
  const allTitlesList = useMemo(() => {
  ...
  const editor = useEditor({
    extensions: [
      ...
      WikiLinkMark.configure({
        isTargetExists: (target) => {
          ...
          return noteTitleSetRef.current.has(target.toLowerCase())
        },
      }),
      // 双向链接 [[ 自动补全：插件回调到外层 React state
      WikiLinkSuggestion.configure({
        onStateChange: (next) => setWikiSuggestState(next),
      }),
    ...
    editorProps: {
      ...
      // ── 编辑器快捷键 ──────────────────────────────────────────────────────────
      // Cmd/Ctrl + 1~6 → 标题；Cmd/Ctrl + 0 → 正文；Cmd/Ctrl + Z → 受保护撤销
      handleKeyDown: (_view, event) => {
        ...
        if (key === 'z' && !event.shiftKey) {
          if (isAtUndoBaseline(ed, undoBaselineRef.current)) {
            event.preventDefault()
            return true
          }
          event.preventDefault()
          safeUndo(ed, undoBaselineRef.current)
          return true
        }

        // 在代码块内不拦截数字快捷键，交还浏览器/输入法默认行为。
        if (event.shiftKey || !ed || ed.isActive('codeBlock')) return false
  ...
  useEffect(() => {
    if (!editor || editor.isDestroyed) return
    // TipTap v3 的 editor.view 是 getter，view 未挂载/已销毁时直接 throw，
    // 仅靠可选链（editor?.view?.dom）救不了。这里 try/catch 兜底。
    let dom
    try {
      dom = editor.view?.dom
    } catch {
    ...
    // 双向链接点击：在 view DOM 捕获阶段拦截，避免被其他 plugin 抢先消化
    const onClickWiki = (e) => {
      try {
        ...
        const fn = onWikiLinkClickRef.current
        if (typeof fn === 'function') fn(target, section, e)
      } catch {}
    }
    dom.addEventListener('click', onClickWiki, { capture: true })

    return () => {
      dom.removeEventListener('dragover', onDragOver, { capture: true })
      dom.removeEventListener('drop', onDrop, { capture: true })
      dom.removeEventListener('click', onClickWiki, { capture: true })
    }
  }, [editor])  // eslint-disable-line react-hooks/exhaustive-deps

  // ── 标题集合变化时，实时刷新 wiki link not-found 状态（DOM 直补，免重渲染整篇）─
  useEffect(() => {
    if (!editor || editor.isDestroyed) return
    let view
    try { view = editor.view } catch { return }
    ...
    if (!dom) return
    try {
      const set = noteTitleSet
      const links = dom.querySelectorAll('a.markdown-wiki-link[data-wiki-target]')
      links.forEach((a) => {
        const t = (a.getAttribute('data-wiki-target') || '').toLowerCase()
        const exists = !t || set.has(t)
        const has = a.classList.contains('markdown-wiki-link-not-found')
        if (!exists && !has) a.classList.add('markdown-wiki-link-not-found')
        else if (exists && has) a.classList.remove('markdown-wiki-link-not-found')
      })
    } catch {}
  }, [editor, noteTitleSet])
  ...
  useEffect(() => {
    ...
    if (noteChanged) {
      // useEditor deps=[noteId]，noteId 变化时会重建实例，新 editor 的初始 content
      // 已在创建时通过 `content: preprocessMarkdown(content ?? '')` 注入；
      // 这里只需重置 ref 状态，不必重复 setContent。
  ...
  const handleEditorContextMenu = useCallback((e) => {
    ...
    let view
    try { view = editor.view } catch { return }
    if (!view?.dom || !view.dom.contains(e.target)) return
    ...
    >
      ...
      <WikiLinkSuggestionPopup
        ...
        onSelect={(title) => {
          ...
          const { from, to } = cur.range
          // 把 `[[query` 替换成 `[[title]]`，光标停在 `]]` 之后
          const insert = `[[${title}]]`
          editor.chain()
            .focus()
            .insertContentAt({ from, to }, insert)
            .run()
          // 立即关闭浮层（state 会被 plugin 检测后再次清，但这里先 UI 上消失）
          setWikiSuggestState({ open: false })
        }}
        onClose={() => setWikiSuggestState({ open: false })}
      />
