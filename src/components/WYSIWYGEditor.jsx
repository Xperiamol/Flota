...
const TextColor = Mark.create({
  ...
  addStorage() {
    return {
      markdown: {
        serialize: {
          open(state, mark) { return `{color:${rgbToHex(mark.attrs.color)}}` },
          close() { return '{/color}' },
        },
        parse: {},
      },
        ...
        props: {
          decorations(state) {
            ...
            state.doc.descendants((node, pos) => {
              ...
              if (!cfg) return
              // 给 blockquote 添加 callout 类名
              decorations.push(
                Decoration.node(pos, pos + node.nodeSize, {
                  class: `callout-block callout-${type}`,
                  'data-callout-type': type,
                  style: `border-left: 4px solid ${cfg.color}; background: ${cfg.color}0d; border-radius: 8px; padding: 10px 14px; margin: 6px 0; font-style: normal; color: inherit;`,
                })
              )
              // 给 [!type] 文本添加 badge 样式
              const firstChildPos = pos + 1 // blockquote 内部开始
              const textStart = firstChildPos + 1 // paragraph 内部开始
              const tagEnd = textStart + match[0].length
              decorations.push(
                Decoration.inline(textStart, tagEnd, {
                  nodeName: 'span',
                  class: 'callout-type-badge',
                  style: `display:inline-block; background:${cfg.color}1a; color:${cfg.color}; border-radius:4px; padding:0 6px; font-size:12px; font-weight:700; font-style:normal; font-family:monospace; margin-right:4px; vertical-align:baseline;`,
                })
              )
            })
...
const WYSIWYGEditor = React.forwardRef(({ content, onChange, placeholder = '开始输入...' }, ref) => {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      editor.commands.setContent(preprocessMarkdown(initial))
    ...
    <Box
      sx={{
        ...
        '& .ProseMirror': {
          ...
          // callout 装饰覆盖：移除默认 blockquote 的斜体和浅色
