...
### Added / 新增
- 新增白板组合图生成管线，支持流程图、时序图、类图、ER 图、思维导图、鱼骨图、甘特图、时间轴、四象限、饼图和架构图的统一编排。
- 新增 Mermaid 图片的 DSL 重画能力，可在白板中选中图像后直接编辑 DSL 并原位替换。

### Changed / 变更
- 重构白板编辑器与 Excalidraw 主题定制，统一顶部工具栏、菜单、属性面板、素材库和滚动条的毛玻璃视觉风格。
...
- 修复白板 AI 追加后图片资源可能丢失、保存失败仍误报成功的问题。
