...
### Changed / 更新内容
- feat: improve todo drag and whiteboard interactions
...
### Added / 新增
- 新增笔记资料包导出，便于携带笔记与关联附件。
- 白板支持插入和编辑 SVG 源码，AI 可结合可编辑元素与 SVG 生成内容。

### Changed / 优化
- 优化 AI 白板文本尺寸、布局和生成提示，精简重复的图表生成逻辑。
- 统一应用图标与列表操作样式，改善笔记、待办及 AI 面板的视觉层级。
- 保留 AI 欢迎页圆球动画、原默认头像与编辑器工具栏的原版操作图标。

### Fixed / 修复
- 修复 AI 辅助输入时的文本选区丢失，调整右键触发交互。
- 改善 AI 待执行操作的确认、重复执行保护及会话切换处理。
- 分离拖拽 Context 与界面依赖，修正热更新时的 Context 一致性问题，并增加回归测试。

## [3.7.4] - 2026-08-27

### Changed / 更新内容
- fix: preserve Windows taskbar pin on upgrade
...
### Changed / 更新内容
- 新增跨端复杂表格版本格式，合并单元格在电脑与手机之间同步时不再丢失结构。
- 旧版 HTML 表格在保存时自动转换为标准 Markdown 表格或 Flota 复杂表格，并保留无法安全转换的原文。
- Markdown 预览支持经过白名单清洗的表格 HTML，电脑端可正确显示手机同步回来的复杂表格。
...
### Changed / 更新内容
- ci: refresh apt metadata before Linux build
...
### Added / 新增
- **笔记多格式导出 / 分享**：编辑器新增导出入口，支持 PNG / PDF / Markdown / HTML 四种格式。PDF/PNG 通过离屏窗口渲染自包含 HTML（图片内联 base64、样式内嵌），导出件脱离应用也能正确显示。新增 `src/utils/noteExport.js`、`electron/ipc/exportHandlers.js` 与 `note:export-document` IPC。
- **笔记内位置书签**：笔记导航面板新增「书签」分区，可标记当前阅读/光标位置并一键跳回，按笔记分组持久化；编辑器右键菜单新增「添加书签」（取选中文字或当前块文本为锚点）。新增 `src/store/useBookmarks.js`。
- **正文全文搜索**：笔记导航面板支持搜索正文内容，基于 CSS Highlight API 高亮命中、列出片段，支持 Enter / Shift+Enter 上下跳转（textarea 与 ProseMirror 双模式），并新增快捷键 `Cmd/Ctrl+F` 唤起。
- **侧边栏自定义排序**：主导航按钮支持拖拽排序（带插入指示线），顺序持久化。新增 `src/store/useSidebarOrder.js`。
- **次级侧边栏可拖拽调宽**：拖拽右边缘调整宽度（220–560px），双击复位，宽度持久化。
- **最近笔记批量关闭**：标签右键新增「关闭以上 / 关闭以下 / 关闭其他」，固定项保留。
- **AI 批量编辑笔记**：新增 `edit_notes` 工具，可一次性整理多条笔记的标题与标签，前端以聚合确认卡逐条勾选后应用；聊天快捷指令更新为整理标题 / 整理标签 / 生成日报 / 写小说。

### Changed / 变更
- **AI 自动整理改为建议制**：切换笔记时 AI 生成的标题不再静默覆盖，改为标题下方「AI 建议标题」供用户采纳，并补充「AI 正在整理…」加载条与失败提示。
- **大纲支持 H1–H6**：笔记大纲从仅 H1–H3 扩展到全部六级标题。
- **图片理解（多模态）设置独立成区**：开关由后端 `visionEnabled` 统一裁决，关闭时同步禁用 `read_note_image` 工具并调整系统提示词，前后端口径一致。
- **AI 待确认操作支持并发执行**：执行态由单一 id 改为 Set，多张确认卡可同时应用。
- **画布图片 / 预览图改为仅本地落盘**：上传交由 SyncEngine 增量去重同步，避免每次保存重复直传。
- **HTML 清洗逻辑抽为共享模块**：新增 `src/markdown/sanitizeHtml.js`，预览与导出复用同一白名单。

### Fixed / 修复
- 修复光标停在顶层位置（图片 / 表格等 atom 节点旁，depth=0）时 `[[` 双链补全 `findQueryAtPos` 抛出 RangeError 导致的崩溃。
- 修复「块级图片 + 标题」被序列化为同一行 `![](src)# 标题`、重载后标题格式丢失的问题（块级图片后补 closeBlock）。
- 修复 AI 待确认操作在重启 / 超 TTL 后用确认卡快照重建动作，历史卡片不再误报「已过期」。
- 修复 AI 设置面板 `ai.configSaved` 等翻译 key 缺失导致的 i18n 告警。
- 新增 AI 流式返回的诊断日志（direct / toolLoop 两条路径记录 finishReason 与内容长度），用于定位浮窗偶发空回复。

### Removed / 清理
- 笔记导航面板移除原「最近笔记」列表，功能由书签与正文搜索取代。
- 移除 `noteSummary` 中重复的取图指令，统一收归系统提示词。
- 移除画布保存时的内联直传逻辑，改由同步引擎处理。
