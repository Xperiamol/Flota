...
const buildCurrentNoteSummary = (note, { headLines = 60, tailLines = 30, maxOutline = 40 } = {}) => {
  ...
  // images 列出笔记里的本地图片引用，供模型决定是否调用 read_note_image；
  // 「是否/如何取图」的指令统一由系统提示词给出，这里不再重复 image_hint。
  const images = extractImageRefs(text);

  if (text.length <= 8000) {
    return { ...base, total_lines: total, total_chars: text.length, content: text, images };
  }
